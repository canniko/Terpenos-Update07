...
interface AddInventoryItemClientProps {
  adminId: number;
}

export default function AddInventoryItemClient({ adminId }: AddInventoryItemClientProps) {
  ...
  const handleCreateProduct = async () => {
    // First save the inventory item, then redirect to product creation
    setLoading(true);
    ...
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        // Redirect to product creation page with the new inventory item ID
        router.push(`/admin/products/new?inventoryId=${result.inventoryItem.item_id}`);
      } else {
        const error = await response.json();
        alert(`Error: ${error.error}`);
      }
              ...
              <div className="flex flex-col gap-4 pt-6">
                ...
                <div className="border-t border-white/20 pt-4">
                  ...
                  <p className="text-sm text-gray-400 mt-2 text-center">
                    This will create the inventory item and then redirect you to create a product linked to it
                  </p>
                </div>
              </div>
            </form>
